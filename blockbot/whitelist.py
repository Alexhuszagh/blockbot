...
def has_friendship(api, source_screen_name, target_screen_name):
...
def should_block_user(api, me, user, whitelist, **kwds):
    ...
    if kwds.get('whitelist_following', True) and user.following:
        ...
        return False
    elif kwds.get('whitelist_friendship', True) and has_friendship(api, user.screen_name, me.screen_name):
    ...
    return not any(has_friendship(api, user.screen_name, i) for i in whitelist)
